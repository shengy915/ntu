...
def change_brightness(image,value):
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            R, G, B = image[i, j]
            image[i, j, :] = np.clip([R + value, G + value, B + value], 0, 255)
...
def magic_wand_select(image, x, thres):
    ...
    while stack:
        ...
        if mask[x, y] == selected_area:
            continue

        if colour_distance(image[x,y], x1) <= thres:
            mask[x, y] = 1
            if x > 0:
                stack.append((x - 1, y))
            if x < c - 1:
        ...
        if choice2 == "e":
            ...
            while True:
              try:
                ...
                print("A non-integer value was entered. Please re-enter value.")
            image = change_contrast(image, value)
            display_image(image, mask)
            ...
            while True:
                try:
                    ...
                    if xx > xy or yx > yy:
                        print("You entered invalid pixel positions. Please enter values such that the pixel position x should be less than or equal to pixel position y.")
                    else:
                        x = (xx,yx)
                        y = (xy,yy)
